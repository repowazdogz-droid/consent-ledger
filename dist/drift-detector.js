...
exports.detectScopeCreep = detectScopeCreep;
...
function getMonetaryValue(parameters) {
    return parseNumber(parameters.amount ?? parameters.value ?? parameters.cost);
}
function getLimitForAuth(auth, type) {
 ...
 */
function detectGradualExpansion(authorisations, actions) {
    const patterns = [];
    ...
    for (const auth of authorisations) {
        ...
        if (limit === null)
            continue;
        const list = (byAuth.get(auth.id) ?? []).slice().sort((x, y) => new Date(x.timestamp).getTime() - new Date(y.timestamp).getTime());
 ...
 */
function detectFrequencyEscalation(authorisations, actions) {
    const patterns = [];
    const byAuth = new Map();
    for (const a of actions) {
        const list = byAuth.get(a.authorisation_id) ?? [];
    ...
    for (const auth of authorisations) {
        const list = (byAuth.get(auth.id) ?? []).slice().sort((x, y) => new Date(x.timestamp).getTime() - new Date(y.timestamp).getTime());
        if (list.length < MIN_DATA_POINTS)
            continue;
        ...
        if (counts.length < MIN_DATA_POINTS)
            continue;
        let escalating = true;
 ...
 */
function detectDomainDrift(authorisations, actions) {
    const patterns = [];
    const byAuth = new Map();
    for (const a of actions) {
        const list = byAuth.get(a.authorisation_id) ?? [];
    ...
    for (const auth of authorisations) {
        ...
        for (const a of list) {
            ...
            if (!inDomain)
                outside.push({ id: a.id, ts: a.timestamp });
        }
 ...
 */
function detectConstraintErosion(_authorisations, actions, violationsByAction) {
    ...
    for (const [constraintType, list] of byType) {
        if (list.length < MIN_DATA_POINTS)
            continue;
        const ordered = list.map((x) => severityOrder[x.severity] ?? 0);
        let increasing = true;
 ...
 */
function detectAuthorityInflation(authorisations, actions, matchStatusByAction) {
    const inflated = actions.filter((a) => ['exceeded', 'revoked', 'expired'].includes(matchStatusByAction.get(a.id) ?? ''));
    if (inflated.length < MIN_DATA_POINTS)
        return [];
...
function detectScopeCreep(input) {
    const { authorisations, actions, violationsByAction = new Map(), matchStatusByAction = new Map(), } = input;
    const all = [];
